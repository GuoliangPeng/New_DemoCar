...
#测试-失真矫正（对原始图像-->失真矫正）
img = mpimg.imread('./20191231test/image0001.jpg')
...
plt.savefig("./writeup_images/image2.png")
plt.show()
'''
'''
#透视变换
...
def pipeline(img, r_thresh=(150, 255), g_thresh=(125,255), h_thresh=(15,120), l_thresh=(210,255),s_thresh=(40,255),l_lab_thresh=(140,200),b_lab_thresh=(225,255),sxy_thresh=(40, 80)):#170, 255
    ...
    g_binary[(g_rgb >= g_thresh[0]) & (g_rgb <= g_thresh[1])] = 1
    ...
    b_lab_binary[((b_lab > b_lab_thresh[0]) & (b_lab <= b_lab_thresh[1]))] = 1
    
    l_lab = lab[:,:,0]
    #np.set_printoptions(threshold=np.inf)
    #print(a_lab)
    l_lab_binary = np.zeros_like(l_lab)
    l_lab_binary[((l_lab > l_lab_thresh[0]) & (l_lab <= l_lab_thresh[1]))] = 1
   ...
   # Combine the two binary thresholds
    ...
    combined_binary[(s_binary==1)&(dir_binary==1)] = 1#(l_lab_binary==1)&(b_lab_binary==1)  (g_binary==1)&(r_binary==1)  (s_binary==1)&(dir_binary==1)  |(sxbinary == 1)&(dir_binary==1)#(s_binary==1)&(dir_binary==1)|(r_binary == 1)|(l_binary == 1)|(sxbinary == 1)#(b_binary == 1)
    return color_binary,combined_binary

#测试-渐变和色彩空间,输出二值化图像(输入图像为已经过失真矫正，透视变换的图像)
test_img = mpimg.imread('./20191231test/2.15ma_imgs/1.jpg')
#color_result_test,combined_binary_test = pipeline(top_down_warped_test)
color_result_test,combined_binary_test = pipeline(test_img)
# Plot the result
f, (ax1, ax2) = plt.subplots(1, 2, figsize=(24, 9))
f.tight_layout()
...
#plt.subplots_adjust(left=0., right=1, top=0.9, bottom=0.)
plt.savefig('./writeup_images/test_image.png')
plt.show()                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     